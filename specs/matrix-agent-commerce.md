...
**It runs the full gate itself**, reusing the router's own lanes rather than reimplementing them: `ContractGateService.check` (one-job-at-a-time → engine record + AuthZ → quota → `maxAmount` → serviceId contracted) then `WorkIntentService.startEngagement` (escrow reservation → durable engagement → per-user replica). On failure nothing is started and the tool returns `{ started: false, reason, message }` with the same reasons the router surfaces — `not_contracted`, `quota_exhausted`, `max_amount_too_low`, `service_not_contracted`, `engagement_in_progress`, `intent_failed` — and the `message` is rendered by the same function that writes the router's gate-failure overlay instruction, so a refusal reads identically whether it happened before the turn or inside it.
