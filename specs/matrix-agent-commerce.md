...
One agent, one checkpointer thread per session, two **modes**. We do not build two separate agents — that would split memory and double infra. The mode changes what the agent is told and, more importantly, what it can physically do:

|                        | Support mode (default)                                                                                                                                                                                           | Work mode (the user has an active engagement)                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Prompt overlay         | "You are the front desk: explain services, prices, status. You do NOT do the work here — not a sample, not a preview. Route with `start_work`."                                                                  | "You are performing service `<serviceId>` under an active contract. Finish with `deliver_work`."                            |
| Tool surface           | **Hard allowlist: the memory plugin + the oracle-payments plugin, and nothing else.** No sandbox, composio, skills, firecrawl, editor, fork tools — and no meta-tools (`load_capability` / `list_capabilities`). | The full runtime surface — memory, sandbox, composio, the fork's own tools, meta-tools, sub-agents.                         |
| Commerce tools exposed | `list_services`, `show_contract`, `get_contract_status`, `get_thread_attachment`, **`start_work`**                                                                                                               | `deliver_work`, `cancel_work` **plus the same read-only four** (so a locked-in user can still ask what they are paying for) |
| Contract gate          | Run by `start_work` itself, per call                                                                                                                                                                             | Already passed; intent escrowed at engagement start (5.1)                                                                   |

**The allowlist is the boundary, not the prompt.** Support mode is enforced in `createMainAgent` at the same seam as the `billing: 'contracted'` filter — right after `registries.tools.collect`, before visibility selection and wrapping — so no later stage ever sees a filtered-out tool. It is keyed by **contributing plugin name** (`RegisteredTool.pluginName`), never by tool name: a fork's tools are work tools whatever they are called. Sub-agents go through the same allowlist, since they share the tool namespace and are themselves ways to do work.

**Everything here applies only to Matrix turns that carry a commerce context.** One guard — `session.client === 'matrix' && requestCtx.commerce` — is resolved once in `createMainAgent` and drives the allowlist, the billing gate, the plugin's per-mode `getRequestTools`, the overlay and the trace name. HTTP, portal and Slack turns, and Matrix turns where the router is inert, keep the full unrestricted surface and their existing prompt. The contracted-billing gate is scoped to that same lane: off the lane there is no engagement to be inside, so a `billing: 'contracted'` tool is just a tool.

**The oracle-payments tools are Matrix-only.** They post `ixo.oracle.component` events into a room, key engagements to thread roots, upload deliveries into the originating room, and read a thread's timeline — so the plugin contributes **no tools at all** on HTTP/portal/Slack. That gate is `session.client === 'matrix'`, deliberately not "the router routed this turn": on a Matrix turn with an inert router (no card published yet) the user can still ask what is on offer and get an honest "nothing published yet".

**Meta-tools and the on-demand machinery are off in support mode.** With a closed surface there is nothing to load, so `load_capability` / `list_capabilities` are not bound, the Tier-1 block covers only the allowlisted plugins and drops its "call `list_capabilities`…" footer, and the Operating-principles discovery mandate is not rendered. Advertising capabilities the agent cannot reach in this mode is worse than not mentioning them.
...
Fork work-tools gating (open decision O3, plain version): a fork's own tools (e.g. `generate_tax_report`) can optionally be marked `billing: 'contracted'` so the runtime hides them from the model until an engagement is active. The allowlist now subsumes most of what that flag bought — on a routed support turn a fork's tools are already gone, flagged or not — so the flag survives as (a) the way an **allowlisted** plugin marks one of its own tools paid, and (b) an explicit declaration that a tool bills. It is scoped to the commerce lane: on an unrouted turn (HTTP or an inert-router Matrix turn) a contracted tool binds like any other, because there is no engagement for it to be inside.
...
- **Classifier (`ORACLE_PAYMENTS_ROUTER_ENGINE=llm`, default):** one structured-output call on `getProviderChatModel('routing')` (provider routing-role default; override via `ORACLE_PAYMENTS_ROUTER_MODEL`). Input today is the coalesced current turn plus the card's service list (`id`, `name`, `description`, `tags`, `examples`); prior-thread context is not currently passed. Output: `{ intent: 'support' | 'work', serviceId?: string, confidence: number }`. Below `0.6` confidence → support (fail-open to the free persona, never accidentally into billable work). The bounded Decision is never evaluated in this mode.
- **Bounded Decision shadow (`decision-shadow`):** the LLM still routes; the same eligible turn also evaluates the boot-registered `oracle-payments.route-message` Decision. The Decision asks a Noul-style work-now question and a finite service Choice. It runs in parallel, is never awaited by the live route, never reaches the contract gate, and logs only probabilities/choice/agreement/latency metadata. Sticky engagements and empty service catalogs skip it. This lane exists to calibrate the live Decision engine below; the LLM classifier remains authoritative in shadow mode.
- **Bounded Decision routing (`decision`):** the Decision routes INSTEAD of the LLM. Its answers are mapped onto the classifier's output shape so the rest of the pipeline is unchanged: `intent = workRequestedNow.probabilityTrue >= 0.6 ? 'work' : 'support'` (the same `0.6` floor as the LLM, so the two engines are comparable and there is no separate Decision threshold); for work, `serviceId = service.value` and `confidence = min(workProbability, service.confidence)`, and the Decision's no-match option falls into the existing unknown-serviceId → support lane; for support, `confidence = 1 - workProbability`. If the Decision cannot answer (no evaluator, no Decision name, provider error, timeout, malformed answer) the router warns once with `engine=decision status=fallback reason=<safe token>` — never the message text — and runs the LLM classifier for that turn only. Sticky engagements and empty service catalogs call neither engine. The decision log line carries `engine=` plus the raw `workProbability=`/`serviceConfidence=` so operators can read back what drove the route.
- **Sticky per USER — no transport-level cancel detection (decided):** while the sender has an `active` engagement anywhere, the classifier is skipped entirely and EVERY message from them goes to the work agent. Stickiness follows the user, not the thread: a bare main-timeline message is its own thread root (matrix-primary F4), so a thread-scoped check dropped live paid work back to the free persona as soon as the user replied outside the thread. The engagement's own room+thread travel on `ctx.commerce` (`engagementRoomId` / `engagementThreadId`) so `deliver_work` / `cancel_work` settle the escrow where the record lives, while the delivered file and the receipt card are posted where the conversation is. Regex/keyword cancel scanning was considered and REJECTED: false positives are catastrophic here (a double-text aborts the in-flight run, then "now edit the report" must simply continue the work, and "cancel the meeting row in the sheet" is an instruction, not a cancellation). Cancellation is an AGENT decision: the work overlay instructs the model — when the user wants to abandon the work, call `cancel_work` (3.2.1). The router never cancels.
...
- **How the mode reaches the agent:** the router result is passed through `deliverHandler` → `sendMessage` metadata → `RuntimeContext` as `ctx.commerce = { mode, engagement? }` (new field, populated by the request preparer from the router's per-turn output — _not_ by a graph middleware; binding rule: middlewares must not mutate state). The prompt composer reads it for the overlay; the plugin's `getRequestTools` reads it to expose the right tool set; `createMainAgent` reads it for the support allowlist. All of them read the SAME guarded value, so the persona in the prompt and the tools in the hand cannot disagree.
- **The classifier never runs for a locked-in user.** `decide` returns on `findActiveEngagement` before `getServices` and before `classify` — that ordering is the contract, not an optimisation. The decision line says so explicitly (`decision=sticky-engagement … classifier=skipped`), so "was this turn classified?" is answerable from a production log.
- **The classifier is not the only way into work mode.** It is the implicit route, and it is deliberately shy (fail-open to support below 0.6 confidence). The explicit route is `start_work` (3.2.2), called by the agent when the user has plainly decided. Both end in the same gate and the same escrow write.
...
#### 3.2.2 `start_work` (support mode only)

The one deliberate, verified transition from the free front desk into paid work. Support mode does no work at all, so entering work must be an act, not an inference from phrasing. Args: `{ serviceId }`, validated against the agent card (an unknown id throws listing the valid ids, exactly as `show_contract` does).

**It runs the full gate itself**, reusing the router's own lanes rather than reimplementing them: `ContractGateService.check` (one-job-at-a-time → engine record + AuthZ → quota → `maxAmount` → serviceId contracted) then `WorkIntentService.startEngagement` (escrow reservation → durable engagement → per-user replica). On failure nothing is started and the tool returns `{ started: false, reason, detail, message }` with the same reasons the router surfaces — `not_contracted`, `quota_exhausted`, `max_amount_too_low`, `service_not_contracted`, `engagement_in_progress`, `contract_check_failed`, `intent_failed` — and the `message` is rendered by the same function that writes the router's gate-failure overlay instruction, so a refusal reads identically whether it happened before the turn or inside it.

`detail` is the failure in words: the chain's rejection text behind an `intent_failed`, the engine's status behind a `contract_check_failed`, the cap-versus-price numbers behind a `max_amount_too_low`. A reason alone is a code the agent can only read back at the user ("the tool responded with `intent_failed`"), so the detail travels on the result and inside `message`, and no failure lane is allowed to end in a log line the agent cannot see.

**Same-turn sequencing (decided).** The tool runs inside a turn whose tool surface was already bound in support mode, and LangChain freezes the bound tool list at compile time — the capability-gate middleware can only filter what is bound, never add to it, and the system prompt is already fixed. So there is no honest way to be in work mode for the rest of that turn, and the runtime does not pretend otherwise: the engagement is live immediately, the **work tools bind on the user's next message** (which routes to work through the sticky-engagement branch, no classification), and the tool's result plus the support overlay both tell the model to confirm the job is open, say what it needs to begin, and produce none of the deliverable in that reply.

### 3.3 Engagements — thread-keyed, one active at a time

**Decided:** engagements are **stored per thread, but looked up per user — only one may be active per user at a time.** The durable record stays in the room state of the thread that started it; a replica of the active engagement, keyed by the user's DID, lives in Redis (`REDIS_URL`) or an in-process map when the oracle has no Redis, and is what answers "work or support?" per turn without a Matrix round-trip and what makes the lookup work from any thread or room. The replica is rebuildable: a miss falls back to the room's durable active-engagement index and repopulates it, and an entry that no longer reads `active` is dropped. The thread root event id is simultaneously the `sessionId`, the checkpointer `thread_id`, and the engagement key. This is why the prompt and the portal both push users into threads: thread = one piece of work = one session = one engagement = (eventually) one claim. What a thread does _not_ buy is concurrency: starting a second paid job while one is still `active` is refused.
...
Gate results cached ~60 s per (room, sender) — **answers only**, never failures, so an engine that comes back is not shut out for another minute. Gate failure never errors the turn — the turn proceeds in support mode with injected context: _"The user asked for `<service>` but holds no usable contract (reason: `<not_contracted | quota_exhausted | max_amount_too_low | service_not_contracted>`). Explain, then call `show_contract` for `<serviceId>`."_ — followed by the `detail` sentence naming what actually blocked it.

Three failure reasons are **not** contracting problems and get their own instruction, without a contract card: `engagement_in_progress` (the user already has a job running — wait or `cancel_work` from that thread, or, when `inProgress.releaseFailed` says an earlier cancellation never reached the chain, retry `cancel_work` there; 3.3), `intent_failed` (the contract is fine, the on-chain reservation failed), and `contract_check_failed` (the lookup never got an answer — engine unreachable, no signing key, unreadable response). The last is deliberately distinct from `not_contracted`: an unanswered check says nothing about the user's contract, so treating it as an absent one tells a paying user they never contracted and sends them a card they do not need.
...
Small dedicated model call — `getProviderChatModel('custom_medium')`, fixed (no env override: the model is a property of the job, not of the deployment) — with structured output:
...
| `ORACLE_PAYMENTS_ROUTER_MODEL`       | provider `routing` role | LLM classifier override (also the `decision` engine's fallback model)                                              |
| `ORACLE_PAYMENTS_ROUTER_ENGINE`      | `llm`                   | `llm` (no Decision), `decision-shadow` (Decision logged only), `decision` (Decision routes, LLM per-turn fallback) |
