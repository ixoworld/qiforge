...
- **Classifier:** one structured-output call on `getProviderChatModel('routing')` (provider routing-role default; override via `ORACLE_PAYMENTS_ROUTER_MODEL`). Input today is the coalesced current turn plus the card's service list (`id`, `name`, `description`, `tags`, `examples`); prior-thread context is not currently passed. Output: `{ intent: 'support' | 'work', serviceId?: string, confidence: number }`. Below `0.6` confidence → support (fail-open to the free persona, never accidentally into billable work).
- **Bounded Decision shadow:** when `ORACLE_PAYMENTS_ROUTER_ENGINE=decision-shadow`, the same eligible turn also evaluates the boot-registered `oracle-payments.route-message` Decision. The Decision asks a Noul-style work-now question and a finite service Choice. It runs in parallel, is never awaited by the live route, never reaches the contract gate, and logs only probabilities/choice/agreement/latency metadata. Sticky engagements and empty service catalogs skip it. This lane exists to calibrate a later Decision-controlled router; the legacy classifier remains authoritative in v1 shadow mode.
