...
export class UcanService implements OnModuleDestroy {
  ...
  // ============================================================================
  // Invocation creation from explicit delegation (skill-driven flow)
  // ============================================================================

  /**
   * Mint a fresh invocation using a delegation supplied at call time.
   *
   * Generic helper for skills that need to call any UCAN-gated service. The
   * caller (typically the `mint_invocation` agent tool) provides the user's
   * delegation CAR; this method resolves the service's did:web from its URL,
   * mints an invocation signed by the oracle with the delegation as proof,
   * and returns it for the caller to forward to the service as a bearer
   * token.
   *
