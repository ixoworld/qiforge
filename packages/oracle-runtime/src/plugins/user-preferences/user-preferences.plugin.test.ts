...
describe('UserPreferencesPlugin', () => {
  ...
  it('contributes exactly one middleware named UserPreferencesMiddleware', () => {
    const plugin = new UserPreferencesPlugin(makeServiceStub());
    const mws = plugin.getMiddlewares(makeBuildCtx());
    expect(mws).toHaveLength(1);
    expect(mws[0]?.name).toBe('UserPreferencesMiddleware');
  });
  ...
  describe('middleware behavior', () => {
    it('populates state.userPreferences from the service when roomId is set', async () => {
      const stored = {
        agentName: 'Yousef',
        language: 'en',
      ...
      expect(result.before).toEqual({ userPreferences: stored });
      await rt.close();
    });

    it('skips when no roomId is present on the runtime context', async () => {
      const get = vi.fn(async () => ({ agentName: 'Yousef' }));
      const rt = await createTestRuntime({
        plugins: [
          new UserPreferencesPlugin(
            makeServiceStub({ get }),
    ...
    it('skips when state.userPreferences is already populated', async () => {
      const get = vi.fn(async () => ({ agentName: 'New' }));
      const rt = await createTestRuntime({
        plugins: [
          new UserPreferencesPlugin(
            makeServiceStub({ get }),
      ...
      const result = await rt.invokeMiddleware(
        'UserPreferencesMiddleware',
        { userPreferences: { agentName: 'Existing' } },
        { context: { session: { roomId: '!room:ixo' } } },
      );
    ...
    it('returns no update when the service returns undefined', async () => {
    ...
    it('swallows service errors and logs a warning', async () => {
