 ...
 *
 * No client outlives a call: the streamable-HTTP transport keeps a
 * server→client stream open for as long as the client exists, and a client
 * cached for the object's lifetime (the previous shape, with `reconnect`)
 * left the user's Durable Object permanently resident — never hibernating,
 * billed around the clock. Each invocation connects, calls, and closes; the
 * upstream tool names are fixed, so nothing needs discovering up front.
 */
export function createDefaultFirecrawlMcpFactory(
  firecrawlUrl: string,
): FirecrawlMcpFactory {
  const withClient = async <T>(
    run: (tools: FirecrawlMcpProxyTool[]) => Promise<T>,
  ): Promise<T> => {
  ...
  const proxy = (name: string): FirecrawlMcpProxyTool => ({
    name,
    invoke: (input) =>
      withClient(async (tools) => {
        const target = tools.find((t) => t.name === name);
        if (!target) {
          throw new Error(
            `upstream MCP tool "${name}" is not exposed by the configured Firecrawl server.`,
          );
        }
        return withCallTimeout(
          () => target.invoke(input),
          FIRECRAWL_TOOL_TIMEOUT_MS,
          name,
        );
      }),
  });
  const tools = [
    proxy(FIRECRAWL_SCRAPE_MCP_NAME),
    proxy(FIRECRAWL_SEARCH_MCP_NAME),
  ];
  return async () => tools;
}
