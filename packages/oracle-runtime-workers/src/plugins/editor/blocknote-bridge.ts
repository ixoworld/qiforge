 ...
 *
 * On workerd, `@blocknote/server-util`'s internal JSDOM is replaced by the
 * linkedom-backed shim in `jsdom-shim.ts` (the `jsdom` specifier is aliased to
 * it — see that file for the alias wiring). Everything else in the pipeline
 * (remark, prosemirror-model, y-prosemirror) is pure JS.
 */
...
/**
 * Building a `ServerBlockNoteEditor` spins up a DOM shim, so it is created
 * once per isolate and shared. It is stateless across calls.
 */
