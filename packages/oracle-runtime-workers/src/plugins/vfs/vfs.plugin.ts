 ...
 * base runtime schema) selects the worker URLs; `VFS_BASE_URL` /
 * `UCAN_STORE_URL` (also base-schema keys, used by the VFS owner store) act
 * as per-deployment overrides; `SANDBOX_MCP_URL` (owned by the sandbox
 * plugin), when set, adds the two sandbox↔files bridge tools so a file can
 * move between the two without its bytes passing through the LLM.
 */
const siblingEnvSchema = z.object({
  ...
  SANDBOX_MCP_URL: z.string().url().optional(),
});
...
export interface VfsPluginOptions {
  /** Injected transport for the VFS client. Defaults to global `fetch`. */
  fetchImpl?: typeof fetch;
  /** Retry backoff forwarded to the VFS client. Tests pass `0`. */
  retryDelayMs?: number;
  /** Injected sandbox MCP-client factory for the two bridge tools. */
  sandboxMcpClientFactory?: SandboxMcpClientFactory;
}
 ...
 * doesn't see the filesystem that turn. Always on — no `autoDetect`.
 */
export class VfsPlugin extends OraclePlugin {
  ...
  private readonly options: VfsPluginOptions;

  constructor(options: VfsPluginOptions = {}) {
    super();
  ...
  override getRequestTools(rtCtx: RuntimeContext): PluginTool[] {
    ...
    const cfg: VfsConfig = {
      VFS_BASE_URL:
        (siblings.success && siblings.data.VFS_BASE_URL) ||
        VFS_DEFAULT_BASE_URLS[network] ||
        // Unreachable: `network` is one of the three keys the map declares.
        'https://vfs.ixo.earth',
      UCAN_STORE_URL:
        ...
        ...createVfsSandboxTools(
          {
            vfsCfg: cfg,
            sandboxMcpUrl,
            vfsFetchImpl: this.options.fetchImpl,
            vfsRetryDelayMs: this.options.retryDelayMs,
          },
          this.options.sandboxMcpClientFactory,
        ),
      );
    }
