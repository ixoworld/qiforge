 ...
 * (composio / memory, and any future minting plugin). Verbatim port of the
 * Node runtime's `plugins/ucan-failure.ts`.
 *
 ...
 * adapter throws a typed `UcanMintUnavailableError` for the EXPECTED cases
 * (no signing key / no cached delegation); anything else is an unexpected
 * failure. We refine the expected case into `no-signing-key` vs
 * `no-delegation` purely for the log message.
 */
