/**
 * Bring-your-own-credential (BYO) provider and model catalog — the Workers
 * port of the Node runtime's `llm/byo-catalog.ts`.
 *
 * When `BYO_LLM_ENABLED` is on, a user can connect their own ChatGPT
 * subscription (OAuth) or per-provider API keys and run their chat turns on
 * their own account instead of the platform's OpenRouter key. This file is
 ...
 *     on the user's provider too (embedding always stays platform-side).
 *
 * Everything here is pure data + pure functions; credential resolution and
 * client construction live in `byo-service.ts` / `byo-client.ts`. Wire shapes
 * (secret names, `byo:` ids) are identical to the Node runtime, so a user's
 * connected credentials survive the runtime swap.
 */
 ...
 * mirroring the platform adapter's behaviour.
 */
 ...
 * `ModelListItem` shape so the client SDK renders them without changes. Tier
 * badge and cost label mirror the platform catalog so BYO rows read the same
 * (the Workers catalog carries no pricing — the user pays their provider
 * directly); the client marks BYO-ness itself from the `byo:` id namespace.
 */
  ...
  for (const provider of connected) {
    for (const entry of BYO_PROVIDER_MODELS[provider]) {
      items.push({
        ...
        blurb: entry.blurb,
        vision: entry.vision,
        // The user pays their own provider; the platform charges nothing per
        // token for a BYO model, so the listed price is zero.
        pricing: {
          inputPerMillion: 0,
          outputPerMillion: 0,
          currency: 'USD',
