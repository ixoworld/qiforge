/**
 * OAuth client for connecting a user's ChatGPT subscription — the Workers
 * port of the Node runtime's `modules/byo-llm/chatgpt-oauth.ts` (the same
 * public client the Codex CLI uses). Two connect paths:
 *
 ...
 *
 * Pure HTTP + parsing; no storage. `WorkersByoService` owns persistence and
 * refresh scheduling. Crypto is bare WebCrypto (no `node:*`), which makes
 * `createPkcePair` async — the only signature difference from the Node file.
 */

import type { ChatGptOAuthTokens } from './byo-catalog';
...
/**
 * Hard timeout on every auth-endpoint call so a stalled upstream never pins
 * a request (or a disconnect awaiting an in-flight refresh) for minutes.
 */
const AUTH_HTTP_TIMEOUT_MS = 15_000;

// ---------------------------------------------------------------------------
// base64url helpers (bare WebCrypto / no Buffer)
// ---------------------------------------------------------------------------
...
/** 32 random bytes, base64url — the `state` for the authorize URL. */
...
function decodeJwtClaims(jwt: string): Record<string, unknown> | null {
  ...
  const payload = base64UrlDecodeToString(payloadSegment);
