import type { OraclePlugin, PluginEnv } from '../plugin-api/oracle-plugin';
...
export interface ResolvePluginsInput {
  ...
  /** Plugins subject to feature toggles + `autoDetect`. */
  bundled: OraclePlugin[];
  /** Plugins that always load (no toggle, no auto-detect). */
  userPlugins?: OraclePlugin[];
  /**
   * The Worker `env` bindings object handed to each plugin's `autoDetect`.
   * Required on Workers — there is no `process.env` to fall back to.
   */
  env: PluginEnv;
  logger?: Logger;
}
