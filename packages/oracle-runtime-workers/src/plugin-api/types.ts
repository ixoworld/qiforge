 ...
 * matched against the Hono router's request path (leading slash optional).
 * This file is a verbatim port of `@ixo/oracle-runtime`'s plugin-api types —
 * the plugin contract is identical across the Node and Workers runtimes; only
 * the NestJS-specific `RequestMethod` type was replaced.
 */
export interface AuthExcludedRoute {
  /** Path under the plugin's controller, e.g. `weather/now`. Leading slash optional. */
  path: string;
  /** HTTP method (`GET`, `POST`, ...). Defaults to all methods when omitted. */
  method?:
    | 'GET'
    | 'POST'
    | 'PUT'
...
export interface RuntimeContext<TConfig = MergedConfig> {
  ...
  /**
   * Host task scheduler for this user, when the runtime provides one (the
   * Workers runtime does; absent elsewhere). Used by the tasks plugin.
   */
  tasks?: OracleTasksSurface;

  /**
   * Host store for the user's per-room behavioural preferences (tone,
   * language, names). Present when the host persists room state; the
   * `set_user_preferences` tool writes through it and the turn preparer
   * hydrates `state.userPreferences` from it before the agent is built.
   */
   ...
   * browser answers. Present when the runtime terminates client sockets
   * (the Workers runtime does); absent on hosts without a realtime channel.
   */
  frontend?: FrontendCallSurface;

  /**
   * Host access to the session's attachments for the `view_attachment`
   * tool: present on hosts that offload old inline payloads (the Workers
   * runtime), absent elsewhere.
   */
  attachments?: AttachmentViewSurface;
  /**
   * Register a cleanup that runs when THIS turn ends — completed, failed or
   * aborted. For anything a tool keeps open across calls within a turn (an
   * MCP client session, a pooled connection): close it here rather than on
   * an idle timer. On Workers a pending timer keeps the user's Durable
   * Object resident (and billed) and blocks WebSocket hibernation, whereas a
   * turn-end close costs nothing.
   */
...
/** How a task run is triggered. */
export type OracleTaskSchedule =
...
export interface OracleTaskRecord {
  ...
  /** `before-action` pauses each run for user approval; `never` runs freely. */
  approval: 'never' | 'before-action';
  createdAt: string;
  updatedAt: string;
  nextRunAt?: string;
  lastRunAt?: string;
  lastResult?: { ok: boolean; summary: string; at: string };
  consecutiveFailures: number;
  /**
   * Dedicated `[Task] <title>` room this task delivers into. Absent → the
   * user's main oracle room (the Node runtime's `delivery.roomId: 'main'`).
   */
...
export interface OracleTaskInput {
  ...
  approval?: 'never' | 'before-action';
  /**
   * Whether the task gets its own `[Task] <title>` room. `auto` (default)
   * applies the Node heuristic: sub-daily cron, long intent, or a
   * monitor/watch/track intent; `before-action` tasks always get one.
   */
  dedicatedRoom?: 'auto' | 'yes' | 'no';
}
...
export interface FrontendCallSurface {
  ...
  callAgAction(params: FrontendCallParams): Promise<unknown>;
  /** Whether at least one authenticated realtime client is attached to the session. */
...
/**
 * Per-user task scheduling surface, implemented by the host runtime (on
 * Workers: a Durable-Object-alarm scheduler whose records live inside the
 * user's own SQLite file). Optional — absent when the host has no scheduler.
 */
export interface OracleTasksSurface {
  preview(
    input: OracleTaskInput,
  ): Promise<{ ok: boolean; nextRuns: string[]; problems: string[] }>;
  create(input: OracleTaskInput): Promise<OracleTaskRecord>;
  list(): Promise<OracleTaskRecord[]>;
  get(id: string): Promise<OracleTaskRecord | null>;
  update(
    id: string,
    patch: Partial<OracleTaskInput>,
  ): Promise<OracleTaskRecord>;
  pause(id: string): Promise<OracleTaskRecord>;
  resume(id: string): Promise<OracleTaskRecord>;
  cancel(id: string): Promise<OracleTaskRecord>;
  /** Resolve a pending before-action approval for the given run. */
  resolveApproval(
    taskId: string,
...
/**
 * Per-room user preferences as the host stores them — the same record the
 * Node runtime's `UserPreferencesService` keeps in the room's
 * `ixo.room.state`/`user_prefs` event. `set` merges: only the provided
 * fields change, and the host stamps `updatedAt`.
 */
export interface UserPreferencesSurface {
  get(roomId: string): Promise<UserPreferences | undefined>;
  set(
    roomId: string,
