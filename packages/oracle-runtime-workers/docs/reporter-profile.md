...
| GET    | `/reporter/sessions/:sessionId`                         | Snapshot and a bounded page of the latest runs, in order.                                                                 |
...
Session pages return `nextCursor: string | null`. Pass that UUID as `?cursor=…` to retrieve older runs. Each page selects the newest remaining runs and returns them in chronological order. Cursors are scoped to the user and session. Pages contain at most 50 runs and at most 261,120 UTF-8 JSON bytes, reserving 1,024 bytes for Reporter freshness metadata. Direct request lookup remains available for every run.

Snapshot, narrative and history each have a 65,536-byte UTF-8 JSON limit; the snapshot limit includes its digest. A run is limited to 194,560 bytes. Text fields are limited to 4,096 characters. Snapshots contain at most 1,000 facts and 200 checks. Narratives contain at most 20 sections, 50 units per section and 30 references per interpretation.
...
The bundled `SKILL.md` has version `1.0.0`. Its packaged string is checked byte-for-byte against the reviewed file. The handler calls the selected BYO model with JSON-schema output and zero automatic retries. It binds no generic tools, sandbox, network research or publishing. Factual validation happens after model generation. Actual provider model and token usage come from the response. Missing metadata or an unknown provider outcome remains uncertain. A response rejected by output validation is failed and retains its available execution and skill receipts. Unknown provider outcomes and cancelled requests remain uncertain. If the provider SDK rejects malformed JSON before exposing model and usage metadata, the runtime also retains uncertainty. A failed response has no accepted narrative.
