...
## Harness outcomes and recovery

`harness_operations` records a fingerprint before an unclassified/write tool starts. A returned successful outcome clears the row; a thrown error or explicit error result leaves it pending. The same fingerprint is blocked across sessions in that user's object until an operator reconciles the external outcome. The record deliberately does not expire automatically. It contains no tool arguments or credentials. This prevents identical replay after a lost result; it is not general exactly-once execution for arbitrary external services or differently expressed arguments.

Operators can inspect `operation_id`, `session_id`, `fingerprint` and `started_at` using the existing authenticated storage inspection workflow. Reconcile with the external service's receipt, then use `SqliteHarnessStore.completeOperation(operationId)` from trusted host tooling. Never clear pending operations merely to make a retry pass. Future service-specific adapters should use downstream idempotency keys and reconcile receipts directly.

`harness_results` and `harness_usage` are stored in the user's existing SQLite copy and follow its owner-store persistence. Result access is session-scoped. Session deletion removes saved results and usage; unresolved operation fingerprints remain to prevent a fresh session from accidentally repeating a write. Usage reports are conservative reservations, not invoices. Keep these tables out of model-visible queries and logs containing raw content.
